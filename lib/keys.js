...
module.exports = function(s3url, options) {
  ...
  var s3config = { signatureVersion: 'v4' };
  ...
  if (!s3) {
    ...
    (new AWS.S3({
      ...
      s3config.region = data.LocationConstraint;
  ...
  keyStream._read = function() {
    if (!ready) return setImmediate(keyStream._read);

    var keepReading = true;
    while (keyStream.cache.length && keepReading) {
      var key = keyStream.cache.shift();
      if (!key) continue;
      var data = options.objectMode ? { Bucket: s3url.Bucket, Key: key } : key + '\n';
      keepReading = keyStream.push(data);
      keyStream.listed++;
    }
