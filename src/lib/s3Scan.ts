...
export class S3Scan {
    ...
    public static Purge(s3Url: string, options: {[key: string]: string | boolean | http.Agent}, callback: (err?: Error) => any): S3ScanPurgeReturn {
        //Todo: Implentation Left
    ...
    public static List(s3Url: string, options: { [key: string]: http.Agent }): Stream.Readable {
        //Todo: Implentation Left
        try {
            const readableStream = new Stream.Readable()
    ...
    public static Scan(s3Url: string, options: {[key: string]: string | boolean | http.Agent}): Stream.Readable {
